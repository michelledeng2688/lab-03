...
public class MainActivity extends AppCompatActivity implements
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        cityList.setOnItemClickListener((parent, view, position, id) -> {
            City clickedCity = dataList.get(position);  // get the city clicked
    ...
    @Override
    public void updateCity(City oldCity, City updatedCity) {
        int index = dataList.indexOf(oldCity);
        if (index != -1) {                      // -1 if old city not found
            dataList.set(index, updatedCity);   // replace the old city
